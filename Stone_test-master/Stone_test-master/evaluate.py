...
--kernel-type 5fold_b3_256_30ep --data-folder original_stone/ --enet-type tf_efficientnet_b3_ns --n-epochs 30 --k-fold 5 --image-size 256
...
def parse_args():
    ...
    parser.add_argument('--batch-size', type=int, default= 16) ## 4 > 16으로 수정
    parser.add_argument('--num-workers', type=int, default= 0) ## default = 0으로 수정
    parser.add_argument('--out-dim', type=int, default=2)
    parser.add_argument('--use-amp', action='store_true')
    parser.add_argument('--use-meta', action='store_true', default= False) ## meta = False 수정
    ...
    parser.add_argument('--k-fold', type=int, default= 5) ## 5
    parser.add_argument('--eval', type=str, choices=['best', 'best_no_ext', 'final'], default="best")
    parser.add_argument('--CUDA_VISIBLE_DEVICES', type=str, default='0')
    parser.add_argument('--n-meta-dim', type=str, default='512,128')
    parser.add_argument('--use-gradcam', action='store_true') ## 추가
...
def main():
    ...
    for fold in folds:
        ...
        ####################################################
        '''
        this_LOGITS, this_PROBS, this_TARGETS = val_epoch_stonedata(model, valid_loader, target_idx, is_ext=df_valid['is_ext'].values, n_test= 5, get_output=True) # n_test = 8 > 3 > 5으로 수정
    ...
    # title별로 그룹화한 평균 예측값 계산
    title_grouped = dfs.groupby('title').agg({'pred': 'mean', 'target': 'first'}).reset_index()

    # 새로운 성능 지표 계산
    Accuracy = (round(title_grouped['pred']) == title_grouped['target']).mean() * 100.
    auc_all_raw = roc_auc_score(title_grouped['target'] == target_idx, title_grouped['pred'])

    # 로그 저장 및 결과 출력
    content = time.ctime() + ' ' + f'Grouped Eval {args.eval}:\nAccuracy : {Accuracy:.5f}\nAUC_all_raw : {auc_all_raw:.5f}\n'
    print(content)
    with open(os.path.join(args.log_dir, f'log_{args.kernel_type}_grouped.txt'), 'a') as appender:
        appender.write(content + '\n')

    # 결과 csv
    title_grouped[['title', 'target', 'pred']].to_csv(
        os.path.join(args.oof_dir, f'{args.kernel_type}_{args.eval}_grouped_oof.csv'), index=True)
    ...
    # 결과 csv 저장
    dfs[['filepath', 'title', 'target', 'pred']].to_csv(os.path.join(args.oof_dir, f'{args.kernel_type}_{args.eval}_oof.csv'), index=True)
    # # dfs[['filepath', 'patient_id', 'target', 'pred']] > dfs[['filepath', 'title', 'target', 'pred']]
