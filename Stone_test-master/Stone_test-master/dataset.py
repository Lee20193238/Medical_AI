...
## line 23: use_meta = True


def get_df_stone(k_fold, data_dir, data_folder, out_dim = 1, use_meta = False, use_ext = False): # use_meta 수정
    '''
    ...
    df_train['filepath'] = df_train['image'].apply(lambda x: os.path.join(data_dir, f'{data_folder}train', x))  # f'{x}.jpg'
    ...
    # patients = len(df_train['title'].unique())
    # print(f'Original dataset의 사람 인원수 : {patients}') # 375
    ...
    print(f'Original dataset의 사람 인원수 : {len(unique_titles)}')
    ...
    df_test['filepath'] = df_test['image'].apply(lambda x: os.path.join(data_dir, f'{data_folder}test', x)) # f'{x}.jpg'
...
def get_meta_data_stoneproject(df_train, df_test):
    ...
    # df_train에 대한 정규화
    df_train['Surface area (um²)'] = scaler.fit_transform(df_train[['Surface area (um²)']])
    df_train['Concentration (pg/um³)'] = scaler.fit_transform(df_train[['Concentration (pg/um³)']])

    # df_test에 대한 정규화 (fit_transform이 아닌 transform을 사용)
    df_test['Surface area (um²)'] = scaler.transform(df_test[['Surface area (um²)']])
    df_test['Concentration (pg/um³)'] = scaler.transform(df_test[['Concentration (pg/um³)']])

# --------------------------------------------------- 열 가져오기

    # df_train.columns에서 모든 정보를 가져옴
    meta_features = ['Surface area (um²)', 'Concentration (pg/um³)'] + [col for col in df_train.columns if col.startswith('site_')]
